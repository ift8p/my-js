...
Для добавления подписи к `figure` используется тег `figcaption`.
...
- `for` - определяет связь между результатом и элементами, используемыми для его вычисления
...
## Другие теги в форме шпаргалок <a name="another"></a>
...
<form>
  ...
  <input
    ...
    name="password"
    pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}"
    title="Пароль не может быть меньше 8 символов и должен содержать одну цифру, одну прописную (заглавную) букву и одну строчную букву"
    required
  />
  <input type="submit" />
</form>
